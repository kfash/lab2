...
public:
    ...
    void move(double tau, double phi) {
        x += vx * tau;
        y += vy * tau;
        z += vz * tau;
        smth= pow(cos(phi)*(x) + sin(phi) * y, 2) + pow(cos(phi)*y - sin(phi) * x, 1) + pow(z, 2);
    }
...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints, double phi) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double smth = pow(cos(phi)*(pointX) + sin(phi) * pointY, 2) + pow(cos(phi)*pointY - sin(phi) * pointX, 1) + pow(pointZ, 2);
            /*if (i == 1)
                {
                printf("smth = %f , %f \n", smth, phi);
                }*/
            double vz = pow(pointX - pointY, 1) / 2;
   ...
   void doTimeStep(double tau,double phi) {
        // По сути метод просто двигает все точки
        for(unsigned int i = 0; i < nodes.size(); i++) {
            nodes[i].move(tau,phi);
        }
    }
...
int main()
{
    ...
    //double tau = 0.01;
    for(unsigned int step = 1; step < 315; step++) {

        mesh.doTimeStep(tau,tau*step);
        mesh.snapshot(step);
